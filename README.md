...
- **GUEST**：游客允许**写入**（上传、删除）的目录
- **GUEST_READ**：游客允许**读取**（查看、下载）的目录
- **账号:密码**：用户的登录凭证，值代表其允许**读写**的目录，用`,`隔开

**重要说明**：
- 如果不设置 `GUEST_READ`，未登录用户将无法查看和下载任何文件
- 如果设置 `GUEST_READ=*`，所有文件对未登录用户可见和可下载
- 用户登录后，可以读写其权限范围内的目录
- **请勿在目录前后加逗号，否则会授予所有目录的权限**

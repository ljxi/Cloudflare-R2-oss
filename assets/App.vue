<template>
  <div class="main" @dragenter.prevent @dragover.prevent @drop.prevent="onDrop">
    ...
    <button class="upload-button circle" @click="showUploadPopup = true">
      <img style="filter: invert(100%)" src="https://cdnjs.cloudflare.com/ajax/libs/material-design-icons/4.0.0/png/file/upload_file/materialicons/36dp/2x/baseline_upload_file_black_36dp.png" alt="Upload" width="36" height="36" @contextmenu.prevent />
    </button>
    <div class="app-bar">
      ...
      <div class="menu-button">
        ...
        <Menu v-model="showMenu" :items="[{ text: '名称A-Z' }, { text: '大小↑' }, { text: '大小↓' }, { text: '粘贴' }]" @click="onMenuClick" />
      </div>
    </div>
    <ul class="file-list">
      <li v-if="cwd !== ''">
      ...
      <li v-for="folder in filteredFolders" :key="folder">
        <div tabindex="0" class="file-item" @click="cwd = folder" @contextmenu.prevent="showContextMenu = true; focusedItem = folder">
          ...
          <div style="margin-right: 10px; margin-left: auto" @click.stop="showContextMenu = true; focusedItem = folder">
            <svg viewBox="0 0 24 24" style="height: 30px; width: 30px"><path fill="currentColor" d="M10.5,12A1.5,1.5 0 1,0 13.5,12A1.5,1.5 0 0,0 10.5,12M10.5,16.5A1.5,1.5 0 1,0 13.5,16.5A1.5,1.5 0 0,0 10.5,16.5M10.5,7.5A1.5,1.5 0 1,0 13.5,7.5A1.5,1.5 0 0,0 10.5,7.5M12,2A10,10 0 1,0 12,22A10,10 0 0,0 12,2Z" /></svg>
          </div>
        </div>
      </li>
        ...
        <div @click="preview(`/raw/${file.key}`)" @contextmenu.prevent="showContextMenu = true; focusedItem = file">
          <div class="file-item">
            ...
            <div style="margin-right: 10px; margin-left: auto" @click.stop="showContextMenu = true; focusedItem = file">
              <svg viewBox="0 0 24 24" style="height: 30px; width: 30px"><path fill="currentColor" d="M10.5,12A1.5,10.5 0 1,0 13.5,12A1.5,1.5 0 0,0 10.5,12M10.5,16.5A1.5,1.5 0 1,0 13.5,16.5A1.5,1.5 0 0,0 10.5,16.5M10.5,7.5A1.5,1.5 0 1,0 13.5,7.5A1.5,1.5 0 0,0 10.5,7.5M12,2A10,10 0 1,0 12,22A10,10 0 0,0 12,2Z" /></svg>
            </div>
          </div>
        </div>
      </li>
...
export default {
  data: () => ({ cwd: new URL(window.location).searchParams.get("p") || "", files: [], folders: [], clipboard: null, focusedItem: null, loading: false, order: null, search: "", showContextMenu: false, showMenu: false, showUploadPopup: false, uploadProgress: null, uploadQueue: [] }),
  computed: {
  ...
  methods: {
    ...
    async fetchFiles() {
      ...
      try {
        let cursor = null;
        do {
          const url = new URL(`/api/children/${this.cwd}`, window.location.origin);
          if (cursor) url.searchParams.set("cursor", cursor);
          const res = await fetch(url);
          if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
          const page = await res.json();
          this.files.push(...(page.value || []));
    ...
    applySort() {
      this.files.sort((a, b) => {
        if (this.order === "大小↑") return a.size - b.size;
        if (this.order === "大小↓") return b.size - a.size;
    ...
    onDrop(ev) {
      const files = ev.dataTransfer.items ? [...ev.dataTransfer.items].filter((item) => item.kind === "file").map((item) => item.getAsFile()).filter(Boolean) : [...ev.dataTransfer.files];
      this.uploadFiles(files);
    },
    onMenuClick(text) {
      if (text === "粘贴") return this.pasteFile();
      this.order = text === "名称A-Z" ? null : text;
      this.applySort();
    },
    ...
    preview(filePath) { window.open(filePath, "_blank"); },
    async pasteFile() {
      if (!this.clipboard) return;
      let newName = window.prompt("重命名为:");
    ...
    async processUploadQueue() {
      if (!this.uploadQueue.length) { await this.fetchFiles(); this.uploadProgress = null; return; }
      ...
      if (file.type.startsWith("image/") || file.type === "video/mp4") {
        try {
          ...
          try {
            await axios.put(`/_api_unused_/${digestHex}`, thumbnailBlob);
          } catch (_) {
            await axios.put(`/api/write/items/_$flaredrive$/thumbnails/${digestHex}.png`, thumbnailBlob);
            thumbnailDigest = digestHex;
          }
        } catch (error) { console.warn("Generate thumbnail failed", error); }
      }
      try {
      ...
      } catch (error) { console.error(`Upload ${file.name} failed`, error); }
    ...
    async moveFile(key) {
      ...
      if (!Number.isInteger(selectedIndex) || selectedIndex < 0 || selectedIndex >= folderOptions.length) return alert("无效的选择");
      ...
      try {
        if (key.endsWith("_$folder$")) {
          ...
          for (const item of allItems) {
            ...
            await this.copyPaste(item.key, targetBasePath + relativePath);
            await axios.delete(`/api/write/items/${item.key}`);
          }
          await this.copyPaste(key, targetBasePath.slice(0, -1) + "_$folder$");
          await axios.delete(`/api/write/items/${key}`);
        } else {
    ...
    uploadFiles(files) {
      ...
      if (this.uploadQueue.length === Array.from(files).length) setTimeout(() => this.processUploadQueue());
    },
  },
  watch: {
    cwd: {
  ...
  created() {
    window.addEventListener("popstate", () => {
      const value = new URL(window.location).searchParams.get("p") || "";
      if (value !== this.cwd) this.cwd = value;
    });
  },
...
<style>
.main { height: 100%; }
.app-bar { position: sticky; top: 0; padding: 8px; background-color: white; display: flex; }
.menu-button { display: flex; position: relative; margin-left: 4px; }
.menu-button > button { transition: background-color 0.2s ease; }
.menu-button > button:hover { background-color: whitesmoke; }
.menu { position: absolute; top: 100%; right: 0; }
</style>
